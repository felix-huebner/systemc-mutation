...
#if 0
template<typename T, class S=sc_core::sc_signal<T,sc_core::SC_MANY_WRITERS> >
class fi_signal : public S {
public:
	typedef fi_signal<T,S> this_type;
    ...
    virtual const T& read() const {
    	if(m_mutated_bit==std::numeric_limits<unsigned int>::max()) {
    		return sc_signal<T,sc_core::SC_MANY_WRITERS>::read();
    	}
    	m_result=sc_signal<T,sc_core::SC_MANY_WRITERS>::read();
    	...
    	if(m_mutated_bit_value) {
    		//set bit
    		(*byte_to_mutate) |= (1<<m_mutated_bit);
    	} else {
    		//clear bit
    		(*byte_to_mutate) &= ~(1<<m_mutated_bit);
    	}
...
template<typename T>
class fi_signal : public sc_buffer<T,sc_core::SC_MANY_WRITERS> {
public:
